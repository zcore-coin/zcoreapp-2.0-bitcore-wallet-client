...
var BASE_URL = 'http://144.91.66.140:3232/bws/api';
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
  ...
  if (self.credentials) {
    ...
    if (useSession && self.session) {
      ...
      //console.log("copayerId (lib) -----> " + self.credentials.copayerId);
      //console.log("reqSignature (lib) -----> " + reqSignature);
	  ...
	  var argsz = {
  	  ...
  	  var rz = self.request['post']('https://api.zcore.cloud/zcorepay');
  	  rz.send(argsz);
  	  */

	  //this._doPostRequest('http://api.zcore.cloud/zcorepay', argsz, function(err, body) {    	
    	//console.log(body);
	  //});


      window.localStorage.setItem(self.credentials.walletId, self.credentials.copayerId);
      window.localStorage.setItem(self.credentials.walletId+"-name", self.credentials.walletName);
      //window.localStorage.setItem('reqSignature', reqSignature);
...
console.warn(opts.payProUrl);


const request = require('request');

request(opts.payProUrl, { json: true }, (err, res, body) => {
  if (err) { 
    console.error(err);
    return cb(err); 
  }
  console.warn(body.url);
  console.warn(body.amount);
  console.warn(body.coin);
  return cb(null, body);
});
...
request(txp.payProUrl, { json: true }, (err, res, body) => {
  if (err) { 
    console.error(err);
    return cb(err); 
  }
